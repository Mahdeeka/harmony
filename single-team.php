...
function format_instagram_url($instagram_input) {
    ...
    // Remove @ symbol if present (both at beginning and end)
    $username = ltrim($instagram_input, '@');  // Remove from beginning
    $username = rtrim($username, '@');         // Remove from end
...
<div class="profile-container <?php echo $isVolunteer ? 'volunteer' : ''; ?>">
    ...
    <h1 class="page-title">أعضاء مجتمع هارموني</h1>
    ...
    <div class="content-wrapper">
        ...
        <div class="profile-right">
            ...
            // Filter out "الإنجازات" tabs for cleaner indexing
            $filteredInfo = array_filter($info, function($item) {
                return $item['title'] !== 'الإنجازات';
            });
            $filteredInfo = array_values($filteredInfo); // Reindex array

            if ($filteredInfo) { ?>
                <div class="tabs-container">
                    <div class="tabs-header">
                        <?php foreach ($filteredInfo as $index => $item) { ?>
...
// Fix Instagram and LinkedIn links if they're still not proper URLs
document.addEventListener('DOMContentLoaded', function() {
    ...
    const allSocialLinks = document.querySelectorAll('.social-link, a[href]:not([href^="http"]):not([href^="mailto"]):not([href^="tel"])');
    ...
    tabPanels.forEach((panel, index) => {
        if (index === 0) {
            ...
            console.log('Panel 0 set as active');
        } else {
            panel.style.display = 'none';
            panel.classList.remove('active');
        }
    ...
    tabButtons.forEach((button, index) => {
        ...
        button.addEventListener('touchend', function(e) {
            e.preventDefault();
            console.log('Tab button touched:', index);
            switchTab(index);
        });
    });
